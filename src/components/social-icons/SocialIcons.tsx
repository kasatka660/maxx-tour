...
// Single source of truth for the site's social / messenger presence.
// The SVG glyphs and the profile URLs used to be copy-pasted into the Topbar,
// MobileNav, Footer and Contacts — edit them here instead.

interface IconProps {
  size?: number;
}

export const TelegramIcon: React.FC<IconProps> = ({ size = 22 }) => (
...
export const ViberIcon: React.FC<IconProps> = ({ size = 22 }) => (
  <svg viewBox="0 0 24 24" width={size} height={size} fill="currentColor">
...
export interface SocialLink {
  // `name` doubles as the CSS modifier suffix (e.g. `messenger-btn--telegram`).
  name: "telegram" | "instagram" | "viber";
  href: string;
  label: string;
...
interface SocialLinksProps {
  // Pixel size of each icon. Defaults to the icons' own 22px.
  size?: number;
  // Class applied to each `<a>` wrapper (e.g. `topbar-social`).
  linkClassName?: string;
}

// Renders the icon-only social links as a bare list of anchors (no wrapper),
// so each consumer can place them inside its own flex container.
const SocialLinks: React.FC<SocialLinksProps> = ({ size, linkClassName }) => (
  <>
